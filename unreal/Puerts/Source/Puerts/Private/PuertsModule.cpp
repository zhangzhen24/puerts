...
public:
	...
	virtual TSharedPtr<puerts::FJsEnv> GetJsEnv() const override
	{
		return JsEnv;
	}
	...
	int32 GetDebuggerPortFromCommandLine()
	{
		...
		static const auto GetPIEInstanceID = [](const TArray<FString>& InTokens) -> int32
		{
			...
			const FString* TokenPtr =
				InTokens.FindByPredicate([](const FString& InToken)
				{
					return InToken.StartsWith(TEXT("GameUserSettingsINI="));
				});
			if (TokenPtr == nullptr)
			{
				return INDEX_NONE;
		...
		const bool bPIEGame = OutSwitches.Find(TEXT("PIEVIACONSOLE")) != INDEX_NONE && OutSwitches.Find(TEXT("game")) !=
			INDEX_NONE;
		if (bPIEGame)
		{
		...
		if (NumberOfJsEnv > 1)
		{
			if (Settings.DebugEnable)
			{
				JsEnvGroup = MakeShared<PUERTS_NAMESPACE::FJsEnvGroup>(NumberOfJsEnv,
				                                                       std::make_shared<
					                                                       PUERTS_NAMESPACE::DefaultJSModuleLoader>(
					                                                       Settings.RootPath),
				                                                       std::make_shared<
					                                                       PUERTS_NAMESPACE::FDefaultLogger>(),
				                                                       DebuggerPortFromCommandLine < 0
					                                                       ? Settings.DebugPort
					                                                       : DebuggerPortFromCommandLine);
			}
			else
			{
...
void FPuertsModule::RegisterSettings()
{
	...
	if (ISettingsModule* SettingsModule = FModuleManager::GetModulePtr<ISettingsModule>("Settings"))
	{
		auto SettingsSection = SettingsModule->RegisterSettings("Project", "Plugins", "Puerts",
		                                                        ...
		                                                        LOCTEXT("TileSetEditorSettingsDescription",
		                                                                "Configure the setting of Puerts plugin."),
